...
def doctor() -> None:
    ...
    row(
        ...
        cfg.gemini_available,
        "set — Gemini 2.5 Flash judge active" if cfg.gemini_available else "not set",
    )
    row(
    ...
    if cfg.gemini_available:
        console.print("[green]Gemini 2.5 Flash configured — using as primary judge.[/green]")
    elif not cfg.digital_ocean_judge_available:
        console.print("[yellow]No judge configured — heuristic judge will be used.[/yellow]")
...
async def _demo(mock: bool, iterations: int) -> None:
    ...
    from evolora.evaluation.gemini_judge import get_gemini_judge
    ...
    # Use Gemini as primary judge, fall back to DigitalOcean, then heuristic
    judge = get_gemini_judge(api_key=cfg.google_api_key) or get_judge(
...
@app.command("smoke-gemini")
def smoke_gemini() -> None:
    """Smoke-test Gemini connectivity (requires GOOGLE_API_KEY)."""
    asyncio.run(_smoke_gemini())


async def _smoke_gemini() -> None:
    from evolora.config import get_config

    cfg = get_config()
    if not cfg.gemini_available:
    ...
    try:
        ...
        console.print(f"[green]Gemini OK (gemini-2.5-flash):[/green] {response.text}")
    except Exception as exc:
        console.print(f"[red]Gemini FAILED:[/red] {exc}")
        raise typer.Exit(1)
