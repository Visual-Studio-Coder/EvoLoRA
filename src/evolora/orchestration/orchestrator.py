...
class Orchestrator:
    ...
    def _get_llm_judge(self):
        if self._llm_judge is None:
            ...
            # Try Gemini first, fall back to DigitalOcean
            gemini = get_gemini_judge(api_key=cfg.google_api_key)
            if gemini:
                self._llm_judge = gemini
            else:
    ...
    def _judge_name(self) -> str:
        if self._judge is None or self._judge.is_mock:
            return "heuristic"
        name = self._judge.__class__.__name__.removesuffix("Judge")
        return name or "LLM"
